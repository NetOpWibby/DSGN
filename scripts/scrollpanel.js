...
(function ($) {
	...
	$.extend(ScrollPanel.prototype, {
		...
		update: function (repeat) {
			...
			self.$viewport.css("height", self.$el.height() + 3 + "px"); // fixes weird 3px space below content
			...
			if (visVertFrac < 1) {
				self.$scrollbar
					.css({
						...
						height: self.$el.innerHeight() + scrollbarHeight - self.$scrollbar.outerHeight(true) + 3 + "px" // adds 3px to scrollbar to compensate for above fix
					})
					.fadeIn(50);
				self.$thumb
					.css({
		...
		onMousedown: function (event) {
			...
			$window.on("mousemove", self.scrollProxy).on("mouseup", function (event) {
